...
func udpLocal(laddr, server, target string, timeout time.Duration,shadow func(net.PacketConn) net.PacketConn) {
	...
	if tgt == nil {
		...
		villog.LogE("UDP target address error: %v", err)
		return
	}

	c, err := net.ListenPacket("udp", laddr)
	if err != nil {
		villog.LogE("UDP local listen error: %v", err)
		return
	}
	defer c.Close()
	...
	villog.LogI("UDP tunnel %s <-> %s <-> %s", laddr, server, target)
	for {
		n, raddr, err := c.ReadFrom(buf[len(tgt):])
		if err != nil {
			villog.LogE("UDP local read error: %v", err)
			continue
		}
		...
		if err != nil {
			villog.LogE("UDP local write error: %v", err)
			continue
		}
	}
}
...
func udpSocksLocal(laddr, server string, timeout time.Duration,shadow func(net.PacketConn) net.PacketConn) {
	...
	for {
		...
		if pc == nil {
			...
			villog.LogI("UDP socks tunnel %s <-> %s <-> %s", laddr, server, socks.Addr(buf[3:]))
			pc = shadow(pc)
...
func UdpRemote(addr string, timeout time.Duration, shadow func(net.PacketConn) net.PacketConn) {
	...
	villog.LogI("listening UDP on %s", addr)
	for {
		n, raddr, err := c.ReadFrom(buf)
		if err != nil {
			villog.LogE("UDP remote read error: %v", err)
			continue
		}

		tgtAddr := socks.SplitAddr(buf[:n])
		if tgtAddr == nil {
			villog.LogE("failed to split target address from packet: %q", buf[:n])
			continue
		}

		tgtUDPAddr, err := net.ResolveUDPAddr("udp", tgtAddr.String())
		if err != nil {
			villog.LogE("failed to resolve target UDP address: %v", err)
			continue
		}
		...
		if err != nil {
			villog.LogE("UDP remote write error: %v", err)
			continue
		}
	}
}
